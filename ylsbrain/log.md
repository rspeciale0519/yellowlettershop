...
- 2026-08-02 [23:20] PRODUCTION RELEASE: applied 3 migrations (orders_refund_columns, order_dispatches, dispatch_uniqueness_and_payment_status) to hosted Supabase lmtpfgfulkynrktdkgpu, verified via list_migrations + cross-checked order_dispatches' RLS-no-policy advisories match the existing webhook_events/background_jobs precedent (not a new gap). Then develop -> main merge (--no-ff, 1cbc0ee), gates re-run fresh on the merged tree (typecheck 0, 269 tests, build 0) before push -- this is the first time the vendor fulfillment loop + inline-payment refactor + money-moment confirmations have been live on app.yellowlettershop.com. Cleanup: deleted merged remote branch feature/vendor-fulfillment, killed stale dev server (PID 37124, had blanked ACCUZIP_API_KEY loaded from earlier smoke test), restarted clean. GOTCHA (self-inflicted, no harm): typed `git branch -d develop` meaning to just switch branches -- caught immediately, `-d` only deletes fully-merged branches so it was a no-op on data (develop was merged into main), `git checkout develop` recreated it from origin at the same SHA. Second GOTCHA: the merge produced a burst of scary-looking IDE diagnostics (broken imports, stale types) that were pure LSP lag from bulk file churn, not real -- tsc came back 0 errors seconds later. Don't trust live diagnostics right after a merge/rebase/branch-switch; always confirm against the real tsc/build run. See journal/2026-08-02.
- 2026-08-03 [00:05] Dev-doc reconciliation post-release — implementation-status.md gained §0 (production release status: what shipped, main had zero lib/fulfillment before, single-Vercel-project identity, main-lags-develop-by-design) and §3b (the 8+1 ultrareview fixes as a table); gates section re-run on the merged main tree; §7 corrected (bare capture/refund routes ARE archived; the 4 debug endpoints are NOT, and are now live in prod). knowledge/{features,roadmap,orientation} + dev-docs/README (v2.1) + CLAUDE.md all de-staled. Scale re-counted rather than carried forward: 37 migrations, 48 public tables (queried information_schema, not inferred), 119 lib modules, 38 test files, 269 tests, 484 components. orientation.md was the worst offender (updated: 2026-05-18, still claimed simulated AccuZip + Redstone doc-only). Scope was the release delta only — §4 PARTIAL / §5 NOT BUILT left as the 2026-07-31 audit found them and are now ~2 days un-reverified. See journal/2026-08-03.
- 2026-08-03 [01:10] Debug endpoints removed + PARTIAL/NOT-BUILT re-audit + prod smoke (fcf0e82 -> dbb1743). FINDING: /api/test-db-verification was answering ANONYMOUS callers in production with HTTP 200 using a SERVICE-ROLE client (RLS bypassed, no auth guard of any kind), leaking table-existence/schema-version/function-count recon; also carried a hard-coded service-role JWT fallback (the public supabase-demo local key, so not a credential leak, but it meant the route could not fail closed). /api/test-db + /test-types also live (200); /api/test-auth-state was correctly NODE_ENV-gated (403). All archived per Rule 1 via git mv (100% renames) to archive/debug-endpoints-2026-08/ w/ evidence README; verified 404 in prod after deploy. LESSON: check "hygiene" items against the live site before filing them as hygiene — this sat in §7 as a footnote across three audits and was actually an unauthenticated service-role endpoint. Re-audit: all 13 PARTIAL rows + the NOT-BUILT list still hold, verified at cited paths (not carried forward); D3 upgraded from "winner declaration unverified" to confirmed absent (no declareWinner in lib/campaigns/). Prod smoke green: schema objects all correct in hosted DB, anonymous 401 on vendors/orders/admin-orders/access-control + PATCH dispatch. NOT DONE: money-path smoke — no prod credentials and Stripe mode undeterminable from outside (pk_ only inlined into authed-page bundles). Mitigation noted: prod vendors table is EMPTY (0 active), so auto-dispatch fails non-fatally and no physical mail would send; residual risk is the card charge alone. See journal/2026-08-03.
- 2026-08-03 [02:05] Redstone replied — §8b inference REFUTED. Per-customer endpoints are DEPRECATED (the spec described a retired process); the generic createOrder endpoint is correct; our API key is "live and valid"; payload needs NO {"Order":…} wrapper (they wrap server-side, so our flat body was always right); and the 500 is "most likely on our end" in brand-new code — our order ids YLS-APITEST-20260801-A..E appear NOWHERE in their logs, so posts die before their internal processes. data/art must be net-downloadable for their cURL step (our signed URLs qualify); the ?token= question is still unanswered. BIG CHANGE: status tracking flipped from blocked-on-them to blocked-on-US — they need a URL from us and will POST automatically once saved to our profile, and their spec defines NO auth for it so we must choose (proposed shared-secret header/HMAC, else high-entropy path token). CANNOT ANSWER his ask for the unique error API ID: every failure was an opaque HTML 500 and redstone-client.ts classifies then DISCARDS the raw body — design lesson: against brand-new third-party APIs, persist raw failure bodies, they are exactly what the vendor asks for later. Handed them a likely cause: we send bleeds/purls/qr_code/streetview/response_boost/api_test as JSON booleans but qty_est/postcardH/postcardW as strings per the spec examples. Reply drafted at docs/temp/redstone-reply-2026-08-03.txt (gitignored). See journal/2026-08-03.
- 2026-08-03 [03:30] Redstone inbound status callback BUILT + SHIPPED (1597caa -> 41e3bcd). Auth = high-entropy secret in the URL PATH, chosen because Travis said their system stores "the endpoint" and nothing else — a header/HMAC means dev work on their side which he quoted in WEEKS, and their team capacity is the binding constraint. Token deliberately NOT treated as sufficient: handler only advances an EXISTING dispatch, forward-only via applyDispatchTransition, Redstone-vendors-only, idempotent, all calls recorded — so a leaked token's worst case is "order marked shipped early", and no money moves (capture is at customer approval). SPEC WIN: read their PDF properly — §4.4 lists their status vocabulary AND offers to map to ours (endpoint accepts BOTH so either choice works), §5 defines the {"fail":false,"msg":"ok"} envelope so their client recognises success. Completed Production -> in_production NOT shipped (printed != mailed; shipped is what emails the customer). FOUND LIKELY 500 CAUSE IN THEIR OWN DOCS: their validation errors include "data may not contain credentials in the URL" and our signed links carry ?token= — now the lead item in the reply. SCHEMA TRAP: admin_audit_log.actor_id is NOT NULL REFERENCES auth.users(id), so a vendor callback would fail the FK and be swallowed by logAdminAction's try/catch — hence vendor_status_callbacks (audit + idempotency + raw bodies); updateDispatchStatus now takes actorId: string|null. TWO SELF-CORRECTIONS: (a) told Rob to avoid plain email for the callback URL — reflexive theatre, Redstone stores it anyway and the endpoint assumes the token leaks; the API KEY is a real credential, the webhook token is a scoped capability, do not conflate. (b) Told him the Vercel CLI was unavailable trusting the SessionStart hook — it IS installed (54.14.1, authed) and set the env var fine; see memory:project-vercel-cli-is-installed, the Vercel MCP has NO env tool. NEAR MISS: .gitignore had `.env.local.bak` exactly so my timestamped `.env.local.bak-<ts>` was NOT ignored; caught via git check-ignore before staging, removed it, widened to `.env.local.bak*`. Gates: typecheck 0, 283 tests, build 0. Migration applied to hosted AND local. See journal/2026-08-03.
- 2026-08-03 [17:45] Truthfulness audit of the Redstone reply found FOUR false claims + a real bug. False: (1) "we keep raw responses" was aspirational — made true via pure buildRawResponse (redacts key from body AND headers, bounds unbounded HTML error pages, 7 tests); (2) "every failure was an HTML 500" — after ~8 posts it switched to JSON "Where did you come from?", split into two facts and handed to Travis as a grep hint; (3) the boolean-vs-string theory REMOVED after extracting spec §8 showed their own example uses our exact typing (api_test boolean, qty_est string) — never send a hypothesis you hold evidence against; (4) "we can take tracking links at the same endpoint" — handler requires a status, said so plainly. BONUS: their spec's JSON example IS {"Order":…}-wrapped, contradicting Travis's "do not wrap" — flagged. THE BUG: I had only tested that a WRONG token 401s, never that the REAL token authenticates — different tests, and a mangled env var would have bounced every post. Testing properly exposed dedupe poisoning: the unmatched path let a FAILED record claim the dedupe key, so a status posted before the order row was visible would make every legitimate retry collide, ack, and be silently dropped forever (same class as payment_transactions). Fixed + verified live (2 retries, both dedupe_key null). RULE: "wrong credential rejected" does NOT verify "right credential accepted" — test both directions. Also committed to main twice by accident after checkout-main-for-deploy; caught by "Everything up-to-date", cherry-picked, main reset. Deployed d14e3dc via `vercel --prod --archive=tgz` (repo exceeds the 15000-file upload cap). Email sent. See journal/2026-08-03.
