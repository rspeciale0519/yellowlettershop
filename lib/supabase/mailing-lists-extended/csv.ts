...
export async function bulkImportRecords(
  ...
  if (!userData?.user?.id) {
    throw new Error('User must be authenticated to import records');
  }
  ...
  for (let i = 0; i < records.length; i += batchSize) {
    ...
    let batchToInsert: Partial<MailingListRecord>[] = batch;

    if (deduplicationField) {
      const fieldValues = batch
        .map((r) => (r as Record<string, any>)[deduplicationField])
        .filter(Boolean);
      ...
      if (fieldValues.length > 0) {
        const { data: existingRecords } = await supabase
          ...
          .in(deduplicationField, fieldValues as string[]);
        ...
        for (const record of batch) {
          const v = (record as any)[deduplicationField];
        ...
        batchToInsert = uniqueRecords;
      } else {
        // nothing to dedupe in this batch
        batchToInsert = batch;
      }
    }

    if (batchToInsert.length > 0) {
      const recordsWithMetadata = batchToInsert.map((record) => ({
        ...record,
