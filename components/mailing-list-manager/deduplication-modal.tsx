...
export function DeduplicationModal({
  ...
  >('address');
  const [matchingStrategy, setMatchingStrategy] = useState<'exact' | 'fuzzy'>(
    'exact'
  );
  const [keepStrategy, setKeepStrategy] = useState<
    'first' | 'last' | 'most_complete'
  >('most_complete');
  const [createBackup, setCreateBackup] = useState(true);
  const [progress, setProgress] = useState(0);
  const [status, setStatus] = useState<
    'idle' | 'scanning' | 'removing' | 'complete' | 'error'
  >('idle');
  ...
  const handleDeduplicate = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        let message = `Failed to deduplicate list (${response.status} ${response.statusText})`;
        try {
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className='sm:max-w-[600px]'>
        <DialogHeader>
          ...
          <DialogDescription>
            Remove duplicate records from your mailing list based on matching
            criteria.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className='space-y-4'>
            ...
            <>
              ...
              <div>
                ...
                <RadioGroup
                  ...
                  className='mt-2'
                >
                  {(['address', 'name', 'phone', 'email'] as const).map(
                    (field) => (
                      <div
                        key={field}
              ...
              <div>
                ...
                <RadioGroup
                  ...
                  className='mt-2'
                >
                  {(['first', 'last', 'most_complete'] as const).map(
                    (strategy) => (
                      <div
                        key={strategy}
                        className='flex items-start space-x-2 mb-2'
              ...
              <div className='flex items-center space-x-2'>
                <Checkbox
                  ...
                  onCheckedChange={(checked) =>
                    setCreateBackup(checked as boolean)
                  }
                />
                <label
              ...
              {createBackup && (
                <Alert>
                  ...
                  <AlertDescription className='text-xs'>
                    A backup of your list has been created in the version
                    history.
                  </AlertDescription>
                </Alert>
              )}
          ...
          {status === 'error' && (
            <Alert className='border-destructive'>
              <AlertCircle className='h-4 w-4' />
              <AlertDescription>
                Deduplication failed. Please try again or contact support if the
                issue persists.
              </AlertDescription>
            </Alert>
          )}
