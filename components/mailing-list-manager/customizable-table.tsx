  ...
  window.addEventListener(
    ...
    (event) => {
      if (event.reason && event.reason.message && event.reason.message.includes("ResizeObserver")) {
        event.stopImmediatePropagation()
        event.preventDefault()
        console.log("Suppressed ResizeObserver promise rejection in customizable-table")
        return false
      }
    },
    true,
...
export function CustomizableTable({
  ...
  const [columnStates, setColumnStates] = useLocalStorage<Record<string, ColumnState>>(
  ...
  useEffect(() => {
    ...
    columns.forEach((column, index) => {
      if (!newColumnStates[column.id]) {
        ...
        updated = true
      } else if (column.id === "rowNumber" && newColumnStates[column.id].width !== 60) {
        ...
        updated = true
      } else if (column.id === "id" && newColumnStates[column.id].width !== 100) {
    ...
    try {
      ...
      if (data.length > 0 && columns.length > 0) {
        ...
        columns.forEach((column) => {
          if (column.autoSize && column.id !== "select" && column.id !== "actions") {
            // Start with header width + padding
            const headerText =
              typeof column.header === "string" ? column.header : column.id.charAt(0).toUpperCase() + column.id.slice(1)
            ...
            if (newColumnStates[column.id] && Math.abs(newColumnStates[column.id].width - maxWidth) > 10) {
  ...
  const visibleColumns = columns
    ...
    .sort((a, b) => (columnStates[a.id]?.order || 0) - (columnStates[b.id]?.order || 0))
  ...
  const isColumnDraggable = (columnId: string) => {
    return columnId !== "select" && columnId !== "actions" && columnId !== "rowNumber" && columnId !== "id"
  }
      ...
      setColumnStates((prev) => {
        ...
        if (draggedOrder < targetOrder) {
          ...
          Object.keys(newStates).forEach((id) => {
            ...
            } else if (newStates[id].order > draggedOrder && newStates[id].order <= targetOrder) {
              newStates[id] = { ...newStates[id], order: newStates[id].order - 1 }
            }
          })
        } else {
          // Moving left
          Object.keys(newStates).forEach((id) => {
            if (id === draggedColumn) {
              newStates[id] = { ...newStates[id], order: targetOrder }
            } else if (newStates[id].order >= targetOrder && newStates[id].order < draggedOrder) {
              newStates[id] = { ...newStates[id], order: newStates[id].order + 1 }
            }
          })
        }
  ...
  return (
    <div className="relative">
      <div className="mb-4 p-2 flex justify-between items-center">
        {showDataOperations && <DataOperationsDropdown columns={columns} />}
        ...
        <div className={showDataOperations ? "" : "ml-auto"}>
          <Button variant="outline" size="sm" onClick={() => setSettingsOpen(true)} className="yellow-hover-button">
          ...
          <table className="w-full border-collapse table-fixed" style={{ minWidth: "100%" }}>
              ...
              <tr>
                {visibleColumns.map((column) => (
                  <th
                    ...
                    } ${column.id === "select" || column.id === "actions" ? "sticky z-20" : ""} ${
                      column.id === "actions" ? "border-r border-gray-200 dark:border-gray-700 px-2" : ""
                    } ${column.id === "select" ? "bg-muted" : column.id === "actions" ? "bg-muted" : "bg-muted/50"}`}
                    style={{
                      width:
                        column.id === "rowNumber"
                          ? "60px"
                  ...
                  >
                    ...
                    <div className="flex items-center">
                      ...
                      {renderHeader ? renderHeader(column, visibleColumns.indexOf(column)) : column.header}
            ...
            <tbody className="divide-y">
              {data.length === 0 ? (
                ...
                data.map((record, index) => renderRow(record, visibleColumns, columnStates, index))
              )}
            </tbody>
          </table>
        </div>
      ...
      <Dialog open={settingsOpen} onOpenChange={setSettingsOpen}>
        <DialogContent className="sm:max-w-md md:max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>Configure column visibility, name format, and other table options.</DialogDescription>
          </DialogHeader>
          ...
          <div className="grid gap-6 py-4">
                ...
                {columns.map((column) => {
                  ...
                  return (
                    <div key={column.id} className="flex items-center space-x-2">
                      ...
                      />
                      <Label htmlFor={`visibility-${column.id}`} className="cursor-pointer">
                        {columnName}
                      </Label>
                    </div>
                  )
              ...
              <div>
                ...
                <div className="space-y-2">
                  <div className="flex items-center space-x-2">
                    ...
                    />
                    <Label htmlFor="name-format-last-first" className="cursor-pointer">
                      Last, First
                    </Label>
                  </div>
                  <div className="flex items-center space-x-2">
                    ...
                    />
                    <Label htmlFor="name-format-first-last" className="cursor-pointer">
                      First Last
                    </Label>
                  </div>
                </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                <Button variant="outline" size="sm" onClick={resetColumnOrder} className="yellow-hover-button">
                  Reset Column Order
                </Button>
                <Button variant="outline" size="sm" onClick={resetColumnVisibility} className="yellow-hover-button">
                  Reset Column Visibility
                </Button>
                <Button variant="outline" size="sm" onClick={handleResetColumnWidths} className="yellow-hover-button">
                  Reset Column Widths
                </Button>
              </div>
            </div>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setSettingsOpen(false)} className="yellow-hover-button">
              Close
            </Button>
          </DialogFooter>
        </DialogContent>
