...
interface LenderAssignedFilterProps {
  criteria: MortgageCriteria
  onUpdate: (values: Partial<MortgageCriteria>) => void
}

export function LenderAssignedFilter({ criteria, onUpdate }: LenderAssignedFilterProps) {
  const { mergeUpdate } = useCriterionUpdate<MortgageCriteria>(onUpdate)
  const [input, setInput] = React.useState("")
  const list = criteria.lenderAssigned || []

  const add = () => {
    const name = input.trim()
    if (!name) return
    if (list.includes(name)) return setInput("")
    mergeUpdate({ lenderAssigned: [...list, name] })
    setInput("")
  }

  const removeAt = (idx: number) => {
    const next = list.filter((_, i) => i !== idx)
    mergeUpdate({ lenderAssigned: next })
  }
  ...
  return (
    <div className="space-y-4 p-4 border-t">
      ...
      <div className="flex gap-2">
        <Input
          placeholder="Add lender/servicer name"
          value={input}
        ...
        {list.length > 0 && (
          <Button variant="ghost" onClick={() => mergeUpdate({ lenderAssigned: [] })}>
            Clear All
          </Button>
        )}
      </div>
