...
export function useDemographicsFilters({ criteria, onUpdate }: UseDemographicsFiltersProps) {
  ...
  const safeCriteria: DemographicsCriteria = { ...defaultCriteria, ...criteria }
  ...
  const generateSelectedCriteria = useCallback(() => {
    ...
    if (safeCriteria.maritalStatus.length > 0) {
      selected.push(`Marital Status: ${safeCriteria.maritalStatus.length} status(es)`)
    }
    ...
    if (safeCriteria.lifestyle.purchasingBehavior.length > 0) {
      selected.push(`Purchasing: ${safeCriteria.lifestyle.purchasingBehavior.length} behavior(s)`)
    }
