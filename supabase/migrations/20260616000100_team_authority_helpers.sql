...
-- p_team_id binds the grant to the resource's OWN team, so a grant minted against
-- another team's (globally-unique) resource_id stays inert — closes the cross-team
-- grant vector from templates / access-requests.
...
grant execute on function public.team_role(uuid), public.is_super_admin(),
  ...
  public.my_resource_permission(text, text, uuid) to authenticated;
