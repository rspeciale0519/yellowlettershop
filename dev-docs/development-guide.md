...
- Production environment variables are managed in Vercel Project Settings (no `.env.production` file in the repo).  
...
// Error boundary component for catching React errors (react-error-boundary)
...
-- Migration: 20240826_add_skip_tracing_tables.sql
...
CREATE TABLE skip_trace_orders (
  ...
  -- Store money as cents (integer) or use a consistent NUMERIC with 2 decimal places
...
This comprehensive setup ensures we catch bugs early and maintain high code quality.

---

## **5. Order Workflow Implementation Priorities**

### **5.1 Development Phases**

Based on the comprehensive order workflow specification, here are the implementation priorities for the development team:

#### **Phase 1: Core Order Infrastructure (Weeks 1-4)**
**Priority**: Critical path for MVP functionality

1. **Order Wizard Framework**
   - `app/orders/new/` directory structure
   - `OrderStepper.tsx` - Step navigation component
   - `OrderProvider.tsx` - React Context for state management
   - Basic step routing and validation

2. **Column Mapping Integration**
   - Integrate existing `components/shared/column-mapping/` system
   - Adapt for order workflow mode vs MLM import mode
   - Ensure YLS field standardization across platform

3. **Contact Card Management**
   - Contact card selection component
   - Inline creation for new users (0 contact cards scenario)
   - Plan-based limits enforcement

4. **Basic Order Review & Payment**
   - Order summary with pricing breakdown
   - Stripe payment authorization (manual capture)
   - Order confirmation generation

#### **Phase 2: Core Workflow Steps (Weeks 5-8)**
**Priority**: Complete end-to-end functionality

1. **AccuZip Validation Integration**
   - Post-column mapping validation step
   - Deliverable/undeliverable record processing
   - Order quantity updates based on validated records

2. **Design Customizer Enhancement**
   - Variable injection from list data + contact cards
   - Live preview with real data (first recipient + sender)
   - Print-ready export (live + variable template versions)

3. **Mailing Options & Campaign Setup**
   - Service level selection (full service, ship to user, print only)
   - Dynamic postage type filtering based on quantity
   - Split campaigns and repeat scheduling

4. **Entry Point Implementation**
   - Template Gallery → Order flow integration
   - MLM → Create Campaign flow
   - List Builder → MelissaData integration

#### **Phase 3: Enhanced UX Features (Weeks 9-12)**
**Priority**: User experience optimization

1. **Save & Resume Capability**
   - Draft order persistence (30-day retention)
   - Auto-save every 30 seconds
   - Resume order dashboard section

2. **Real-time Pricing & Validation**
   - Running total throughout workflow
   - Quantity threshold notifications
   - Smart suggestions for optimization

3. **Error Recovery Systems**
   - AccuZip validation failure handling
   - Payment authorization error recovery
   - Network connectivity resilience

4. **Reorder Workflow**
   - Streamlined reorder with edit options
   - Previous settings pre-population
   - Quick approval path

#### **Phase 4: Advanced Features (Weeks 13-16)**
**Priority**: Power user and automation features

1. **Advanced Analytics Integration**
   - Order workflow abandonment tracking
   - Step-by-step performance metrics
   - Conversion optimization data

2. **Proof Review System**
   - PDF annotation interface
   - Collaborative review workflow
   - Approval triggers for payment capture

3. **Order Status Tracking**
   - Real-time order progression
   - Vendor integration status updates
...
```typescript
// Standardized order step component interface
...
- React Context for order-wide state
...
interface OrderWorkflowIntegrations {
  columnMapping: ColumnMappingData
  accuZipValidation: ValidationResults
  melissaDataPurchase: ListPurchaseData
  fpdDesignState: FPDDesignData
...
- Column mapping with live preview
- AccuZip validation integration
- Contact card requirement enforcement
- Basic design customizer integration
- Stripe payment authorization

#### **Performance Targets**
- Order completion rate >75% from start
- Average workflow completion <10 minutes
- Step abandonment <15% per step
- Payment authorization success >98%

#### **Quality Gates**
- End-to-end testing for all entry point flows
...
- Shared utilities in `lib/orders/`
- Type definitions in `types/orders.ts`
- API routes in `app/api/orders/`

#### **Testing Strategy**
- Unit tests for all workflow step components
- Integration tests for complete flows
- Mock data for consistent testing
- Performance testing for large list handling
...
This implementation roadmap ensures systematic development of the complete order workflow system while maintaining code quality and user experience excellence.  
