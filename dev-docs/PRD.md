...
#### **Standardized Order Workflow (All Entry Points)**
**Required Steps:** List Data Decision → Column Mapping → AccuZip Validation → Contact Cards → Design Customizer → Mailing Options → Campaign Setup → Review → Payment

#### **Order Entry Points**
1. **Dashboard "Create New Order"** - Primary entry point for all users
2. **Template Gallery "Use Template"** - Template pre-loaded in Design Customizer
3. **Mailing List Manager "Create Campaign"** - List data pre-selected
4. **List Builder "Purchase & Mail"** - MelissaData list building + purchase
5. **Previous Orders "Reorder"** - Streamlined with previous settings pre-filled
6. **Quick Order "One-Off Mail"** - Single recipient simplified flow
7. **Design Tool "Save & Order"** - Custom design converts to template flow

#### **Core Workflow Steps**

* **Step 1: List Data Decision**
  * User chooses: Use mailing list data (Yes/No)
  * If Yes: Upload new file, select from MLM, or manual entry
  * If No: Skip to Contact Cards

* **Step 2: Column Mapping** (if using list data)
  * Full-page dedicated interface with live preview
  * Manual field mapping with smart auto-detection
  * Required field validation and data quality checks
  * Maps user columns to YLS standard fields for variable injection

* **Step 3: AccuZip Validation** (if using list data)
  * USPS address validation for deliverability scoring
  * Updates order quantity based on validated deliverable records
  * Provides validated/undeliverable records report

* **Step 4: Contact Cards Selection**
  * Mandatory for all orders (required for postal compliance)
  * Choose existing or create new contact card
  * Plan-based limits enforced (Free: 1, Pro: 2, Team/Enterprise: pooled)

* **Step 5: Design Customizer**
  * FPD integration with live variable preview
  * Real recipient + sender data for accurate visualization
  * Support for templates, custom designs, and personalization
  * Variable injection from both list data and contact cards

* **Step 6: Mailing Options Configuration**
  * Service level: Full mail service, Ship to user, Print only
  * Dynamic postage type selection based on validated quantities
  * Shipping address configuration for applicable options

* **Step 7: Campaign Setup**
  * Split campaign configuration (drops and intervals)
  * Repeat campaign options with scheduling
  * Campaign calendar visualization

* **Step 8: Review & Approval**
  * Complete order summary with pricing breakdown
  * Design lock confirmation with no-refund acknowledgment
  * Terms and conditions acceptance

* **Step 9: Payment Authorization**
  * Stripe integration with manual capture workflow
  * Funds authorized but not captured until proof approval
  * Support for stored payment methods

#### **Enhanced UX Features**

* **Save & Resume**: Draft order persistence with 30-day expiration
* **Real-time Pricing**: Running total visible throughout workflow
* **Error Recovery**: Graceful handling of validation failures
* **Smart Defaults**: Remember user preferences and auto-fill
* **Progressive Disclosure**: Basic vs Advanced options toggle
