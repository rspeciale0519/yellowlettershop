...
#### **Primary Action Zones**
**Order Creation Entry Points:**
- **"Create New Order"** - Primary CTA button leading to standardized order workflow
- **"Use Template"** - Quick access to template gallery with direct order flow
- **"Reorder Previous"** - Fast reorder from order history with streamlined approval
- **"Send Single Mail"** - One-off mailing simplified workflow

#### **Quick Access Panels**
- **Draft Orders** - Resume incomplete orders with 30-day persistence
- **Recent Templates** - Continue working on saved designs
- **Active Campaigns** - Monitor ongoing mail campaigns
- **Pending Proofs** - Orders awaiting user approval
...
#### **Navigation to Order Entry Points**
| Entry Point | Dashboard Element | Target Workflow |
|-------------|-------------------|-----------------|
| **Dashboard → Create New Order** | Primary CTA button | Full workflow: List Data → Column Mapping → AccuZip → Contact Cards → Design → Options → Review → Payment |
| **Dashboard → Templates** | Template gallery link | Template pre-loaded: List Data → Column Mapping → AccuZip → Contact Cards → Design (template loaded) → Options → Review → Payment |
| **Dashboard → Mailing Lists** | MLM quick access | List pre-selected: Column Mapping → AccuZip → Contact Cards → Design → Options → Review → Payment |
| **Dashboard → List Builder** | Demographics targeting | MelissaData purchase → AccuZip validation → Contact Cards → Design → Options → Review → Payment |
| **Dashboard → Order History** | Recent orders table | Reorder flow: List Decision → Contact Cards (confirm) → Review (with edit options) → Payment |
