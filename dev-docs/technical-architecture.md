...
> for what is actually built, partial, or dropped. Known-superseded here: FPD, Prisma, NextAuth.js, Jest/Cypress (project uses Mocha+RTL), 4-role model, Sentry.
...
* **Column Mapping System** - Full-page interface for CSV field mapping with live preview
  * Location: `components/shared/column-mapping/`
  * Features: Smart auto-detection, manual override, data validation, real-time preview
  * Integration: Order workflow + MLM import with consistent UX
  * Export formats: Live data + variable templates for print-ready output
...
* **AccuZIP REST API** - CASS-certified address validation
  ...
  * **Integration Point**: Post-column mapping validation in order workflow

* **MelissaData API** - Targeted mailing list building and purchase
  * Demographics-based list building with property criteria
  * Geographic targeting (state, county, city, ZIP, radius)
  * Property characteristics filtering (value, size, type, history)
  * **Integration Point**: List Builder "Purchase & Mail" workflow
  * **Output**: Raw list data passed to AccuZIP for validation
