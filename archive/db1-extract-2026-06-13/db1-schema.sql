-- DB1 (jgkkcr) public schema, extracted via Management API.
...
do $$ begin if not exists (select 1 from pg_constraint where conname='user_profiles_stripe_customer_id_key' and connamespace='public'::regnamespace) then alter table public.user_profiles add constraint user_profiles_stripe_customer_id_key UNIQUE (stripe_customer_id); end if; end $$;
do $$ begin if not exists (select 1 from pg_constraint where conname='user_profiles_pkey' and connamespace='public'::regnamespace) then alter table public.user_profiles add constraint user_profiles_pkey PRIMARY KEY (id); end if; end $$;
do $$ begin if not exists (select 1 from pg_constraint where conname='user_profiles_id_fkey' and connamespace='public'::regnamespace) then alter table public.user_profiles add constraint user_profiles_id_fkey FOREIGN KEY (id) REFERENCES auth.users(id) ON DELETE CASCADE; end if; end $$;
do $$ begin if not exists (select 1 from pg_constraint where conname='user_profiles_team_id_fkey' and connamespace='public'::regnamespace) then alter table public.user_profiles add constraint user_profiles_team_id_fkey FOREIGN KEY (team_id) REFERENCES teams(id); end if; end $$;
do $$ begin if not exists (select 1 from pg_constraint where conname='user_profiles_role_check' and connamespace='public'::regnamespace) then alter table public.user_profiles add constraint user_profiles_role_check CHECK ((role = ANY (ARRAY['free_user'::text, 'pro_user'::text, 'team_member'::text, 'team_manager'::text, 'enterprise_member'::text, 'enterprise_manager'::text, 'admin'::text, 'super_admin'::text]))); end if; end $$;
do $$ begin if not exists (select 1 from pg_constraint where conname='user_profiles_subscription_plan_check' and connamespace='public'::regnamespace) then alter table public.user_profiles add constraint user_profiles_subscription_plan_check CHECK ((subscription_plan = ANY (ARRAY['free'::text, 'pro'::text, 'team'::text, 'enterprise'::text]))); end if; end $$;
do $$ begin if not exists (select 1 from pg_constraint where conname='user_profiles_subscription_status_check' and connamespace='public'::regnamespace) then alter table public.user_profiles add constraint user_profiles_subscription_status_check CHECK ((subscription_status = ANY (ARRAY['active'::text, 'cancelled'::text, 'past_due'::text, 'unpaid'::text]))); end if; end $$;
...
CREATE INDEX IF NOT EXISTS idx_user_profiles_subscription_plan ON public.user_profiles USING btree (subscription_plan);
CREATE INDEX IF NOT EXISTS idx_user_profiles_team_id ON public.user_profiles USING btree (team_id);
CREATE UNIQUE INDEX IF NOT EXISTS user_profiles_pkey ON public.user_profiles USING btree (id);
CREATE UNIQUE INDEX IF NOT EXISTS user_profiles_stripe_customer_id_key ON public.user_profiles USING btree (stripe_customer_id);
...
create policy "Users can insert own profile" on public.user_profiles as permissive for insert to public with check ((id = auth.uid()));
drop policy if exists "Users can update own profile" on public.user_profiles;
create policy "Users can update own profile" on public.user_profiles as permissive for update to public using ((id = auth.uid()));
drop policy if exists "Users can update their own profile" on public.user_profiles;
create policy "Users can update their own profile" on public.user_profiles as permissive for update to public using ((auth.uid() = id));
drop policy if exists "Users can view own profile" on public.user_profiles;
create policy "Users can view own profile" on public.user_profiles as permissive for select to public using ((id = auth.uid()));
drop policy if exists "Users can view their own profile" on public.user_profiles;
create policy "Users can view their own profile" on public.user_profiles as permissive for select to public using ((auth.uid() = id));
